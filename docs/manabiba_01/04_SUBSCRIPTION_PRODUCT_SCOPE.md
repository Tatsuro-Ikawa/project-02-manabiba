...
| 区分 | ログイン | 課金 | メモ |
|------|----------|------|------|
| **ゲスト** | 不要 | — | 未認証。公開コンテンツの閲覧中心。**7日間プログラム導線は非表示**（§5.1）。 |
...
コース遷移の一覧（前コース×後コース×スタート時実施・フリー期間・課金・データ保持・必要 UI）は **プロダクト表「サブスクコース選択の変化」** を正とする。実装フェーズでは **画面先行 → Firestore 手動／開発用切替で検証 → Stripe（Phase C）** の順とする（§9）。
