...
- `coach_client_assignments/{assignmentId}` - コーチ↔クライアント割当（A-11）
- `communication_board_threads/{threadId}` および `…/messages/{messageId}` - **メッセージボード**（read: 割当当事者、write: ルールで拒否＝API 経由のみ）
