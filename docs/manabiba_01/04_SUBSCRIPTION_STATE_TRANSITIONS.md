...
- マネジメント: ゲスト・フリー（7日間スタートのみ含む）は **セクション自体を出さない**（案内文もなし）。Aiコース・プレミアムで `HomeDashboard` を表示（[04_HOME_SCREEN_IMPLEMENTATION.md](./04_HOME_SCREEN_IMPLEMENTATION.md) §1）。
