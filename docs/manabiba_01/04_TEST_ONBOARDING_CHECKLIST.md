...
版の判定は `src/lib/consent.ts` の `TERMS_VERSION` / `PRIVACY_VERSION`（現状 `2026-03-18`）と一致している必要がある。
...
| 7-7 | **`/start-program` に直行** | **二重同意画面なし**。ヘッダー「スタートプログラム」、7日間ダミー本文 |OK |
...
| # | 手順 | 期待結果 | OK |
|---|------|----------|-----|
| 7-R1 | `/consent` のスクロール内容 | 「第2章　7日間スタートプログラム」見出しが含まれる（フリー利用者向け） | |
| 7-R2 | 同上 | 「第3章　気づきノート」見出しも含まれる（有料利用時の読み分け用・同一画面） | |
...
| ログイン済み | 気づきノートを続ける | `/trial_4w`（ランディング省略・要検討は別件） |
...
| S-6 | サイドバー **実行** | `/trial_4w`、実行が active |OK |
