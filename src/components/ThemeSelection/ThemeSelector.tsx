...
import { UserType, ThemeEntry } from '@/types/themeSelection';
import {
  ID_ATTRIBUTES,
  CONTENT_ATTRIBUTES,
...
export const ThemeSelector: React.FC<ThemeSelectorProps> = ({
  ...
  // デバッグ用ログ
  console.log('ThemeSelector - userType:', userType);
  console.log('ThemeSelector - entries:', entries);
    ...
    return [...entries].sort((a, b) => {
      ...
      switch (sortField) {
        ...
        case 'category':
          ...
          break;
        case 'desire':
          aValue = a.metrics.desire || 0;
          bValue = b.metrics.desire || 0;
          break;
        case 'excitement':
          aValue = a.metrics.excitement || 0;
          bValue = b.metrics.excitement || 0;
          break;
        case 'feasibility':
          aValue = a.metrics.feasibility || 0;
          bValue = b.metrics.feasibility || 0;
          break;
        default:
          return 0;
      }
    ...
    if (userType === 'aspiration') {
      return (
        <div className="bg-green-50 p-4 rounded-lg">
          <h4 className="font-semibold text-green-800 mb-2">選択のアドバイス</h4>
          <ul className="text-sm text-green-700 space-y-1">
            <li>• 「これが自分にとって大切そう」と感じるものを選んでみましょう</li>
            <li>• 無理に完璧なテーマを選ばなくてOKです</li>
            <li>• 「一番小さく取り組めそうなもの」を選んでみましょう</li>
            <li>• 「気になったけどスルーできなかった項目」がヒントかも</li>
            <li>• 「感情が大きく動いた項目」は良い起点になります</li>
          </ul>
        </div>
      );
    } else {
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        >
          <table 
            {...createDataAttributes({
              'data-id': ID_ATTRIBUTES.THEME_SELECTOR_TABLE,
          ...
          >
            <thead>
              <tr className="bg-gray-50">
                ...
                <th 
                  className="border border-gray-300 p-3 text-left text-sm font-medium text-gray-700 cursor-pointer hover:bg-gray-100 select-none"
                  onClick={() => handleSort('excitement')}
                >
                  <div className="flex items-center space-x-1">
                    <span>ワクワク度</span>
                    {getSortIcon('excitement')}
                  </div>
                </th>
                <th 
                  className="border border-gray-300 p-3 text-left text-sm font-medium text-gray-700 cursor-pointer hover:bg-gray-100 select-none"
              ...
              {sortedEntries.map((entry, index) => {
                ...
                >
                  ...
                  <td className="border border-gray-300 p-3">
                    {editingIndex === originalIndex ? (
                      <select
                        ...
                        className="w-full p-2 border border-gray-300 rounded text-sm text-gray-900"
                      >
                        <option value="自己成長">自己成長</option>
                        <option value="夢・願望">夢・願望</option>
                        <option value="挑戦">挑戦</option>
                        <option value="人間関係での在り方">人間関係での在り方</option>
                      </select>
                    ) : (
                  ...
                  <td className="border border-gray-300 p-3 text-center">
                    {editingIndex === originalIndex ? (
                      <select
                        value={editingData?.metrics.desire || 0}
                        onChange={(e) => handleEditChange('metrics.desire', parseInt(e.target.value))}
                        className="w-full p-2 border border-gray-300 rounded text-sm text-gray-900"
                      >
                      ...
                      <span className={`font-medium text-sm ${getMetricColor(entry.metrics.desire || 0)}`}>
                        {entry.metrics.desire || 0}
                      </span>
                    )}
                  </td>
                  <td className="border border-gray-300 p-3 text-center">
                    {editingIndex === originalIndex ? (
                      <select
                        value={editingData?.metrics.excitement || 0}
                        onChange={(e) => handleEditChange('metrics.excitement', parseInt(e.target.value))}
                        className="w-full p-2 border border-gray-300 rounded text-sm text-gray-900"
                      >
                        <option value={1}>1</option>
                        <option value={2}>2</option>
                      ...
                      <span className={`font-medium text-sm ${getMetricColor(entry.metrics.excitement || 0)}`}>
                        {entry.metrics.excitement || 0}
                      </span>
                    )}
                  </td>
                  <td className="border border-gray-300 p-3 text-center">
                    {editingIndex === originalIndex ? (
                      <select
                        value={editingData?.metrics.feasibility || 0}
                        onChange={(e) => handleEditChange('metrics.feasibility', parseInt(e.target.value))}
                        className="w-full p-2 border border-gray-300 rounded text-sm text-gray-900"
                      >
                        <option value={1}>1</option>
                        <option value={2}>2</option>
                      ...
                      <span className={`font-medium text-sm ${getMetricColor(entry.metrics.feasibility || 0)}`}>
                        {entry.metrics.feasibility || 0}
                      </span>
                    )}
                  </td>
                  <td className="border border-gray-300 p-3 text-center">
